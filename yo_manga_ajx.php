...
$keyword = esc_html($_GET['term']);

$search_value = sanitize_text_field($keyword);

$table = $wpdb->posts;

$sql = $wpdb->prepare("SELECT *  FROM $wpdb->posts WHERE post_title LIKE %s and post_status = 'publish' and post_type = 'post' order by post_title limit 20","$search_value%");

$posts_resutls = $wpdb->get_results( $sql , ARRAY_A );
